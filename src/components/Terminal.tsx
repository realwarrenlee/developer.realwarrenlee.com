...
export const Terminal: React.FC = () => {
  const { state, input, setInput, isTyping, executeCommand, showMatrix, setShowMatrix, showFire, setShowFire, showDonut, setShowDonut, showTetris, setShowTetris, showHack, setShowHack, showPong, setShowPong, showSnake, setShowSnake } = useTerminal();
  ...
  return (
    <>
      ...
      {showTetris && (
        <TetrisGame onComplete={() => setShowTetris(false)} />
      )}
      {showHack && (
        <HackEffect onComplete={() => setShowHack(false)} />
      )}
      {showPong && (
        <PongGame onComplete={() => setShowPong(false)} />
      )}
      {showSnake && (
        <SnakeGame onComplete={() => setShowSnake(false)} />
      )}
